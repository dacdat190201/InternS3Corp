import React from 'react';
import { Rating } from '@mui/material';
import { Link } from 'react-router-dom';
import './Category.css';
import '../listProduct/Product/ListProduct.css';
const Category = ({ props }) => {
    console.log(props);
    return (
        <div className="data__container">
            {props &&
                props.products.map((item, key) => {
                    return (
                        <div className="main__product" key={key}>
                            <div className="main_product-icon">
                                <svg
                                    width="24"
                                    height="24"
                                    viewBox="0 0 24 24"
                                    fill="none"
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                    <path
                                        d="M8 5C5.7912 5 4 6.73964 4 8.88594C4 10.6185 4.7 14.7305 11.5904 18.8873C11.7138 18.961 11.8555 19 12 19C12.1445 19 12.2862 18.961 12.4096 18.8873C19.3 14.7305 20 10.6185 20 8.88594C20 6.73964 18.2088 5 16 5C13.7912 5 12 7.35511 12 7.35511C12 7.35511 10.2088 5 8 5Z"
                                        stroke="black"
                                        strokeWidth="1.5"
                                        strokeLinecap="round"
                                        strokeLinejoin="round"
                                    />
                                </svg>
                            </div>
                            <Link to={`/products/${item.id}`} style={{ textDecoration: 'none' }}>
                                <div className="main_product-img">
                                    <img src={item.images[0]} width={350} height={300} alt="product1" />
                                </div>
                                <div className="main_product-body">
                                    <h4>{item.title}</h4>
                                </div>
                            </Link>
                            <div>
                                <div className="main_product-bottom">
                                    <Rating name="simple-controlled" value={item.rating} id="simple-controlled" />

                                    <span className="main_product-price">${item.price}</span>
                                </div>
                            </div>
                            <div className="main__product__btn">
                                <div className="main__product-btn1">
                                    <button>Add To Cart</button>
                                </div>
                                <div className="main__product-btn2">
                                    <button>Add Shortlist</button>
                                </div>
                            </div>
                        </div>
                    );
                })}
        </div>
    );
};

export default Category;
